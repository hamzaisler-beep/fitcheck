    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <header style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', padding: '10px 0' }}>
         <div style={{ 
          ...
          <div style={{ textAlign: 'center' }}>
            <h2 style={{ fontSize: '22px' }}>{auth.currentUser?.displayName || 'Kullanıcı'}</h2>
      ...
      <section>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
           ...
           <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Henüz veri yok</span>
        </div>
        <div className="glass-card" style={{ padding: '20px', height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {weightData.length > 0 ? (
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>İlerleyişini görmek için tartıl ve kaydet!</p>
          )}
        </div>
      </section>
        ...
        {!hasReport ? (
          ...
          <div className="glass-card" style={{ padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
             <div style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', borderRadius: '14px' }}>
                ...
                <h4 style={{ fontSize: '16px', color: 'var(--accent-primary)' }}>-%</h4>
             </div>
             <div style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', borderRadius: '14px' }}>
                <p style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Kas Kütlesi</p>
                <h4 style={{ fontSize: '16px', color: 'var(--accent-secondary)' }}>- kg</h4>
             </div>
             <div style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', borderRadius: '14px' }}>
             ...
             <div style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', borderRadius: '14px' }}>
                ...
                <h4 style={{ fontSize: '16px' }}>- kcal</h4>
             </div>
             <button style={{ gridColumn: 'span 2', marginTop: '8px', fontSize: '13px', color: 'var(--accent-primary)', fontWeight: '600', background: 'none', border: 'none' }} onClick={() => setHasReport(false)}>
               Yeni Rapor Yükle
             </button>
          </div>
        )}
      ...
      <section>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <button style={{ color: 'var(--accent-primary)', fontSize: '14px', fontWeight: '600', background: 'none', border: 'none' }}>Güncelle</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div className="glass-card" style={{ padding: '20px', display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            <div>
              ...
              <h4 style={{ fontSize: '18px' }}>{userData?.weight || '0'} <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>kg</span></h4>
            </div>
          </div>
          <div className="glass-card" style={{ padding: '20px', display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            <div>
              ...
              <h4 style={{ fontSize: '18px' }}>{userData?.height || '0'} <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>cm</span></h4>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <MenuItem icon={<TrendingUp size={20} />} label="Başarımlar" badge="0" />
...
const MenuItem = ({ icon, label, badge }: any) => (
  <div className="glass-card" style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
    ...
    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
      {badge !== undefined && badge !== "0" && <span style={{ padding: '2px 8px', background: 'var(--accent-tertiary)', borderRadius: '10px', fontSize: '11px', fontWeight: 'bold' }}>{badge}</span>}
